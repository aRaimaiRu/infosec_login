            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-light" >
                <div class="container-fluid flexstr" >
                    <div className="flexitem">
                        {data.name?`ชื่อ ${data.name}`:""} {data.surname?`นามสกุล ${data.surname}`:""} {data.address?`ที่อยู่ ${data.address}   `:""}
                        {data.status?`status:${data.status}`:''}
                    </div>

                    <div className="flexitem">
                    {(situation==2)&&<button type="button" id="sidebarCollapse" class="btn btn-info" onClick={()=>{Contact()}} disabled={isAlreadyContact}>
